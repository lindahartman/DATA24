...
Here’s how it works: Figures should have labels that start with `fig-`. Tables should have labels that start with `tbl-`.
...
If a cell produces a table, you should label it with `tbl-`, like this:
...
```
#| tbl-cap: "Summary statistics for `gapminder` dataset."
```

Then, in your text, you can refer to the table using @tbl-summary-stats, and Quarto will handle the rest — linking it properly and ensuring consistent formatting.
...
- Add labels and captions to your figures (fig-) and tables (tbl-).
- Insert a Markdown cell under each figure or table, explaining key insights.
- Refer to figures and tables using @fig-label or @tbl-label in your text.
